...
b = [
    '¿Para qué usamos las snake case?', 'Para crear nombres de variables con espacios', 'Para crear nombres de variables con varias palabras', 'No las usamos', '2',
    'Una snake case es la convención que compone las palabras separadas por barra baja'
]
c = [
    '¿Cómo sabemos que un valor es una string?', 'es una palabra entre comillas dobles', 'Contiene el signo"="', 'Son números', '1',
    'Las string, son un tipo de datos compuestos por secuencias de carácteres que representan texto'
]
d = ['¿Cuál de estas lineas de código actualiza la variable "Status"?','status == "Working out"','status += "Working out"','status = "Working out"','3','Para modificar el valor de una variable en Python, basta con asignarle un nuevo valor en cualquier momento y lugar después de la definición']
e = ['¿Cuál es un buen uso para los valores "True" y "False"?','Almacenamiento de valores de uno al cinco','Mostrar si una función está activada o desactivada','Repetir valores eternamente','2','En Python cualquier variable (en general, cualquier objeto) puede considerarse como una variable booleana. En general los elementos nulos o vacíos se consideran False y el resto se consideran True.']
f = ['Elige el que sea mejor para mostrar a un usuario que se dio de baja de un servicio.','subscribed = False','subscribed = True','subscribed = 10','1','Una variable booleana es una variable que sólo puede tomar dos posibles valores: True (verdadero) o False (falso)']
